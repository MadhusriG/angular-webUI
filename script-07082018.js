    ...
    app.controller('newHomeController', function ($scope, $rootScope, $http, $timeout, $location, $interval) {
        $interval.cancel($rootScope.dumpStatusPromise);
        $interval.cancel($rootScope.upgradeStatusPromise);
        $rootScope.titleHeader = 'Power Distribution';
    //	$rootScope.sfcmCount = 0;
        $rootScope.finalSfcmCount = 0;
        $rootScope.sfcmCount = 0;
        $rootScope.bcmCount = 0;
        $rootScope.objPDU;
        $rootScope.objBCM;
        $rootScope.objSFCM;
        $rootScope.bcmPanelOnArr = [];
        $rootScope.sfcmPanelOnArr = [];
        $rootScope.SFCMSubfeedStatusArr = [];
        $rootScope.bcmDeviceLabel;
        var subfeedNum = 0;
        //var sfcmDeviceId = $rootScope.objSFCM.Id;
        
        function identifyBCM(bcmDevice)
        { 
            for(var bcmPanel=1;bcmPanel <= 4;bcmPanel++){
               identifyBCMPanelOn(bcmDevice.Id,bcmPanel);
               //console.log($rootScope.bcmPanelOnArr);
            }
            /*$http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDevice.Id)+"/1"
            }).then(function mySuccess(response) {
                  $scope.BCM = response.data;
        //          console.log("BCM dta:" +response.data);
                  console.log($scope.BCM);
                  if($scope.BCM.PanelConfig.OnOff === "On"){
                      $rootScope.bcmPanelOnArr.push("1");
                      console.log("BCM Panel On Array:" +$rootScope.bcmPanelOnArr);
                      $rootScope.bcmCount++;
                      console.log("bcm count:" +$rootScope.bcmCount);
                  }
                return response;
          ...
          $http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDevice.Id)+"/2"
            }).then(function mySuccess(response) {
                  $scope.BCM = response.data;
        //          console.log("BCM dta:" +response.data);
                  console.log($scope.BCM);
                  if($scope.BCM.PanelConfig.OnOff === "On"){
                      $rootScope.bcmPanelOnArr.push("2");
                      console.log("BCM Panel On Array:" +$rootScope.bcmPanelOnArr);
                      $rootScope.bcmCount++;
                      console.log("bcm count:" +$rootScope.bcmCount);
                  }
          ...
          $http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDevice.Id)+"/3"
            }).then(function mySuccess(response) {
                  $scope.BCM = response.data;
        //          console.log("BCM dta:" +response.data);
                  console.log($scope.BCM);
                  if($scope.BCM.PanelConfig.OnOff === "On"){
                      $rootScope.bcmPanelOnArr.push("3");
                      console.log("BCM Panel On Array:" +$rootScope.bcmPanelOnArr);
                      $rootScope.bcmCount++;
                      console.log("bcm count:" +$rootScope.bcmCount);
                  }
          ...
          $http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDevice.Id)+"/4"
            }).then(function mySuccess(response) {
                  $scope.BCM = response.data;
        //          console.log("BCM dta:" +response.data);
                  console.log($scope.BCM);
                  if($scope.BCM.PanelConfig.OnOff === "On"){
                      $rootScope.bcmPanelOnArr.push("4");
                      console.log("BCM Panel On Array:" +$rootScope.bcmPanelOnArr);
                      $rootScope.bcmCount++;
                      console.log("bcm count:" +$rootScope.bcmCount);
                  }
        ...
        {
            for(var bcmPanelOn = 0;bcmPanelOn < $rootScope.bcmPanelOnArr.length;bcmPanelOn++){
              $rootScope.bcmDeviceLabel = bcmDeviceId + "/" + $rootScope.bcmPanelOnArr[bcmPanelOn];
              console.log("BCM Label:"+$rootScope.bcmDeviceLabel);
          }
        }
        
        function identifyBCMPanelOn(bcmDeviceId,bcmPanelId)
        {
            return $http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDeviceId)+"/"+parseInt(bcmPanelId)
            }).then(function mySuccess(response) {
                  $scope.BCM = response.data;
        //          console.log("BCM dta:" +response.data);
                  console.log($scope.BCM);
                  if($scope.BCM.PanelConfig.OnOff === "On"){
                      $rootScope.bcmPanelOnArr.push(bcmPanelId);
                      console.log("BCM Panel On Array:" +$rootScope.bcmPanelOnArr);
                      $rootScope.bcmCount++;
                      console.log("bcm count:" +$rootScope.bcmCount);
                      bcmDeviceLabel(bcmDeviceId,$rootScope.bcmPanelOnArr);
                  }
            }, function myError(response) {
              $scope.BCM = response.data;
          });
        ...
        {   
            for(var sfcmPanel=1;sfcmPanel <= 4;sfcmPanel++){
           identifySFCMPanelOn(sfcmDevice.Id,sfcmPanel);
           //console.log($rootScope.sfcmPanelOnArr);
            }
            /*$http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?SubfeedSystemData/submeters/"
                +parseInt(sfcmDevice.Id)+"/1"
                        }).then(function mySuccess(response) {
                          $scope.SFCM = response.data;
                //          console.log("SFCM dta:" +response.data);
                          console.log($scope.SFCM);
                          if($scope.SFCM["Config"]["On-Off"] === "On"){
                              $rootScope.sfcmPanelOnArr.push("1");
                              console.log("sfcm Panel On Array:" +$rootScope.sfcmPanelOnArr);
                          }
                        }, function myError(response) {
                          $scope.SFCM = response.data;
                      });
                      $http({
                            method : "GET",
                            url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?SubfeedSystemData/submeters/"
                +parseInt(sfcmDevice.Id)+"/2"
                        }).then(function mySuccess(response) {
                          $scope.SFCM = response.data;
                //          console.log("SFCM dta:" +response.data);
                          console.log($scope.SFCM);
                          if($scope.SFCM["Config"]["On-Off"] === "On"){
                              $rootScope.sfcmPanelOnArr.push("2");
                              console.log("sfcm Panel On Array:" +$rootScope.sfcmPanelOnArr);
                          }
                        }, function myError(response) {
                          $scope.SFCM = response.data;
                      ...
                      $http({
                            method : "GET",
                            url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?SubfeedSystemData/submeters/"
                +parseInt(sfcmDevice.Id)+"/3"
                        }).then(function mySuccess(response) {
                          $scope.SFCM = response.data;
                //          console.log("SFCM dta:" +response.data);
                          console.log($scope.SFCM);
                          if($scope.SFCM["Config"]["On-Off"] === "On"){
                              $rootScope.sfcmPanelOnArr.push("3");
                              console.log("sfcm Panel On Array:" +$rootScope.sfcmPanelOnArr);
                          }
                        }, function myError(response) {
                          $scope.SFCM = response.data;
                      ...
                      $http({
                            method : "GET",
                            url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?SubfeedSystemData/submeters/"
                +parseInt(sfcmDevice.Id)+"/4"
                        }).then(function mySuccess(response) {
                          $scope.SFCM = response.data;
                //          console.log("SFCM dta:" +response.data);
                          console.log($scope.SFCM);
                          if($scope.SFCM["Config"]["On-Off"] === "On"){
                              $rootScope.sfcmPanelOnArr.push("4");
                              console.log("sfcm Panel On Array:" +$rootScope.sfcmPanelOnArr);
                              console.log("sfcm Panel On Array Length:" +$rootScope.sfcmPanelOnArr.length);
                          }
                        }, function myError(response) {
                          $scope.SFCM = response.data;
                      });*/
                
                //var sfcmPanelOnArray = [2,4];
                //for(var sfcmPanelOn = sfcmPanelOnArray.length - 1; sfcmPanelOn >= 0; sfcmPanelOn--){
                    //for(subfeedNum = 1;subfeedNum <= 15; subfeedNum++){
                        //identifySubFeedOnCTScale(sfcmPanelOnArray[sfcmPanelOn], subfeedNum);
                        /*if(subfeedNum <= 14){
                        identifySubFeedOnCTScale(sfcmPanelOnArray[sfcmPanelOn], subfeedNum).then(function(response) {

                            $scope.SFCMPanelSubFeed = response.data;
                            //console.log("SFCM Panel ID:"+sfcmPanelOnId);
                            //console.log("Subfeed ID:"+subfeed);
                            //console.log($scope.SFCMPanelSubFeed);
                            console.log('Bik - '+$scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale)
                               if($scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0){
                               //console.log($scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0);
                               if(subfeedNum<=8)
                               {
                                   $rootScope.sfcmCount++; 
                                   console.log("sf count:" +$rootScope.sfcmCount);
                                   identifySubFeedStatus(sfcmPanelOnId, subfeed).then(function(statusResponse){
                                        $scope.SFCMPanelSubFeedStatus = statusResponse.data;
                                        console.log($scope.SFCMPanelSubFeedStatus);
                                        console.log($scope.SFCMPanelSubFeedStatus.Others.Status);
                                        $rootScope.SFCMSubfeedStatusArr.push($scope.SFCMPanelSubFeedStatus.Others.Status);
                                        console.log("sf status:" +$rootScope.SFCMSubfeedStatusArr);                                                  
                                  });
                               }
                               } 
                            //$rootScope.sfcmCount = $rootScope.sfcmCount+res;
                        });
                      }*/
                    //}
                    /*console.log("subfeed count for Panel:"+sfcmPanelOnArray[sfcmPanelOn]+' - '+$rootScope.sfcmCount);
                    $rootScope.finalSfcmCount = $rootScope.finalSfcmCount + $rootScope.sfcmCount;
                    console.log("final sfCount:" +$rootScope.finalSfcmCount);
                    $rootScope.sfcmCount = 0;*/
                //}
            }
        
        function identifySFCMPanelOn(sfcmDeviceId,sfcmPanelId)
        {
            return $http({
                method : "GET",
                url : "http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?SubfeedSystemData/submeters/"
                +parseInt(sfcmDeviceId)+"/"+parseInt(sfcmPanelId)
                        }).then(function mySuccess(response) {
                          $scope.SFCM = response.data;
                //          console.log("SFCM dta:" +response.data);
                          console.log($scope.SFCM);
                          if($scope.SFCM["Config"]["On-Off"] === "On"){
                              $rootScope.sfcmPanelOnArr.push(sfcmPanelId);
                              console.log("sfcm Panel On Array:" +$rootScope.sfcmPanelOnArr);
                              console.log($rootScope.sfcmPanelOnArr);
                          }
                          for(var sfcmPanelOn = $rootScope.sfcmPanelOnArr.length - 1; sfcmPanelOn >= 0; sfcmPanelOn--){
                            for(subfeedNum = 1;subfeedNum <= 15; subfeedNum++){
                                identifySubFeedOnCTScale($rootScope.sfcmPanelOnArr[sfcmPanelOn], subfeedNum);
                            }
                          }
                        }, function myError(response) {
                          $scope.SFCM = response.data;
        ...
        function identifySubFeedOnCTScale(sfcmPanelOnId, subfeed)
        {
                  return $http({
                            method: "GET",
                            url: 'http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/24/'+parseInt(sfcmPanelOnId)+'/'+parseInt(subfeed)
                        }).then(function mySuccess(response){
                            $scope.SFCMPanelSubFeed = response.data;
                            console.log("SFCM Panel ID:"+sfcmPanelOnId);
                            console.log("Subfeed ID:"+subfeed);
                            console.log($scope.SFCMPanelSubFeed);
                               if($scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0 && $rootScope.sfcmCount < 16){
                               console.log($scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0);
                               $rootScope.sfcmCount++; 
                               console.log($rootScope.sfcmCount);
                               identifySubFeedStatus(sfcmPanelOnId, subfeed); 
                               } 
    //                        return response;    
                        },function myError(response){
                           $scope.SFCMPanelSubFeed = response.data;
        ...
        function identifySubFeedStatus(sfcmPanelOnId, subfeed)
        {
                  return $http({
                            method: "GET",
                            url: 'http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/24/'+parseInt(sfcmPanelOnId)+'/'+parseInt(subfeed)
                        }).then(function mySuccess(response){
                            $scope.SFCMPanelSubFeedStatus = response.data;
                            console.log($scope.SFCMPanelSubFeedStatus);
                            console.log($scope.SFCMPanelSubFeedStatus.Others.Status);
                            $rootScope.SFCMSubfeedStatusArr.push($scope.SFCMPanelSubFeedStatus.Others.Status);
                            console.log("sf status:" +$rootScope.SFCMSubfeedStatusArr);
    //                      return response;
        ...
        $scope.drawSvgCircuit = function()
       {
            //$scope.svg_pduLoad();
            if($rootScope.bcmCount === 0 && $rootScope.sfcmCount === 0){
               $scope.svg_pduLoad(); 
            }
            else{
                $scope.svg_pduWithLineLoad($rootScope.objPDU.devciestatus);
                /*testing purpose*/
                /*for(var b=0;b < 2;b++){
                    $scope.svg_bcmLoad($rootScope.objBCM.devciestatus);
                }
                for(var s=0;s < 16;s++){
                    $scope.svg_sfcmLoad($rootScope.SFCMSubfeedStatusArr[s]);
                }*/
                for(var b=0;b < $rootScope.bcmCount;b++){
                    $scope.svg_bcmLoad($rootScope.objBCM.devciestatus);
                }
                for(var sfStatus = 0;sfStatus <= $rootScope.SFCMSubfeedStatusArr.length - 1;sfStatus++){
                    $scope.svg_sfcmLoad($rootScope.SFCMSubfeedStatusArr[sfStatus]);
                }
            }
       }
        ...
        $scope.invokeDeviceStatusService = function() {
               return $http({
                method : "GET",
                url : "http://" + HOSTNAME + ':' + PORT + "/getdata.cgi?devicestatus"
              }).then(function mySuccess(response) {
                      $scope.deviceStatusObj = response;
                      console.log(response.data);
                      console.log($scope.deviceStatusObj.data.devicestatus.length);
                      for(var i = $scope.deviceStatusObj.data.devicestatus.length - 1;i >=0; i--){
                            switch($scope.deviceStatusObj.data.devicestatus[i].Type)
                            {
                                case "PDU":
                                $rootScope.objPDU = $scope.deviceStatusObj.data.devicestatus[i];
                                break;
                                case "BCM":
                                $rootScope.objBCM = $scope.deviceStatusObj.data.devicestatus[i];
                                    console.log($rootScope.objBCM);
                                break;
                                case "SFCM":
                                $rootScope.objSFCM = $scope.deviceStatusObj.data.devicestatus[i];
                                    console.log($rootScope.objSFCM);
                                break;
                            }
                        }
                      //return response.data;
                      console.log($rootScope.objPDU);
                      console.log($rootScope.objBCM);
                      console.log($rootScope.objSFCM);
                      identifyBCM($rootScope.objBCM);
                      identifySFCM($rootScope.objSFCM); 
                  
                  /*identifyBCM($rootScope.objBCM).then(
                    function mySuccess(bcmResponse){
                        identifySFCM($rootScope.objSFCM).then(
                            function mySuccess(sfResponse){
                                $scope.drawSvgCircuit();
                            },function myError(sfResponse) {
                                $scope.sfResponse = sfResponse.data;
                            });
                    },function myError(bcmResponse) {
                        $scope.bcmResponse = bcmResponse.data;
                    });*/
                   //return response;
                }, function myError(response) {
                  $scope.deviceStatusObj = response.data;
              }); 
       ...
       //service or function to draw the SVG mimic: END
        ...
        $scope.svg_sfcmLoad = function(deviceStatus){
                           ...
                           $("#bs-container").append(svgSFCMWrapper);
                            ...
                            sfLabel.setAttribute("y",'185.00764');
        ...
        $scope.svg_bcmLoad = function(deviceStatus){
                           ...
                           var svg_bcm = document.createElementNS("http://www.w3.org/2000/svg", 'svg'); //create svg element
                            ...
                            bcmBottomLine.setAttribute("height","60.529999");                    bcmBottomLine.setAttribute("width","1.5119047");
                            ...
                            bcmLabel.setAttribute("style","font-size:16px;line-height:1.25;fill:#ffffff;fill-opacity:1;display:inline;");
                            ...
                            bcmId.setAttribute("style","font-size:16px;line-height:1.25;fill:#ffffff;fill-opacity:1;display:inline;");
                            ...
                            bcmIdText.append(bcmId);
                            bcmIdText.setAttribute("id","bcmLabelId");
                            bcmIdText.textContent = "7/1";
        ...
        $scope.invokeDeviceStatusService().then(
       ...
       /*$timeout(function () {
        ...
        /*$scope.drawSvgCircuitPromise = $interval(function () {
            $scope.drawSvgCircuit();
        }, DATADELAY);

        $scope.$on('$destroy', function () {
            $interval.cancel($scope.drawSvgCircuitPromise);
        });*/
    ...
    app.controller('metersHomeController', function($scope, $http, $timeout, $location, $interval, $rootScope) {
        $interval.cancel($rootScope.dumpStatusPromise);
        $interval.cancel($rootScope.upgradeStatusPromise);
        ...
        $scope.options = {
						...
						scales: {
                            xAxes: [{
                                gridLines: {
                                    display: false, //grid lines of x axis are gone. Since by default display is true 
                                }
                            }], 
                            yAxes: [{
                                gridLines: {
        ...
        $scope.invokeLoadChartData = function(){
                ...
                .success(function(response) {
							...
							$scope.panelData1 =  $scope.loadResponse1.Meter.Load;
