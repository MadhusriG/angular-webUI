    ...
    app.controller('newHomeController', function ($scope, $rootScope, $http, $q, $timeout, $location, $interval) {
        ...
        $scope.identifySFOnCTScale = function(sfcmDevice,sfcmPanelOnId){
            ...
            function pushsubFeedNumTest(subfeedNum){
                for(var i = 0;i <= subfeeds.Subfeed.length-1;i++){
                    ...
                    //subfeeds.Subfeed[i].push(subfeedObj.finalSubfeed);
                }
            }
        ...
        $scope.identifySFStatus = function(sfcmDevice,subfeed){
            ...
            var defer4 = $q.defer();
            var promises = [];
            //console.log(subfeeds.SubFeed);
            //for(var sfcmPanelOn = 0;sfcmPanelOn <= sfcmPanelOnId.length-1;sfcmPanelOn++){ 
                subfeeds.SubFeed.finalSubfeed.forEach(function(sf){
                    ...
                    promises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters'+'/'+parseInt(sfcmDevice.Id)+'/'+parseInt(sf.SubFeed.panelId)+'/'+parseInt(sf.SubFeed.finalSubfeed.subFeedId)))
                });
            //}
