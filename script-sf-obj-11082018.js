                ...
                //subfeeds.SubFeed.finalSubfeed.forEach(function(sf){
                  for(var i=0;i <= subfeeds.Subfeed.length-1;i++){
                      for(var j=0;j <= subfeeds.SubFeed[i].finalSubfeed.length-1;j++){ 
                        ...
                        promises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters'+'/'+parseInt(sfcmDevice.Id)+'/'+parseInt(subfeeds.SubFeed[i].panelId)+'/'+parseInt(subfeeds.SubFeed[i].finalSubfeed[j])))
                      }
                });
            //}
