    ...
    //	$rootScope.sfcmCount = 0;
        ...
        { 
            /*var chainBCM = $q.when();
            for(var i = 0; i <= 4; i++) {
                chainBCM = chainBCM.then(function() {
                    return $http.get("http://" + HOSTNAME + ":" + PORT + "/getdata.cgi?meters?PanelConfig/submeters/"
    +parseInt(bcmDevice.Id)+"/"+parseInt(i));
            ...
            return $http({
                ...
                //          console.log("SFCM dta:" +response.data);
                          ...
                          for(var sfcmPanelOn = $rootScope.sfcmPanelOnArr.length - 1; sfcmPanelOn >= 0; sfcmPanelOn--){
                            for(subfeedNum = 1;subfeedNum <= 14; subfeedNum++){
    ...
    //                        return response;    
        ...
        function identifySubFeedStatus(sfcmPanelOnId, subfeed)
        {
                  return $http({
                            ...
                            var subfeedIdStatus = {
                              "subFeedId": subfeed,
                              "status": $scope.SFCMPanelSubFeedStatus.Others.Status
                            };
                            //$rootScope.SFCMSubfeedStatusArr.push($scope.SFCMPanelSubFeedStatus.Others.Status);
                            $rootScope.SFCMSubfeedStatusArr.push(subfeedIdStatus);
                            console.log($rootScope.SFCMSubfeedStatusArr);
    //                      return response;
                        },function myError(response){
                           $scope.SFCMPanelSubFeedStatus = response.data;
                        });
        ...
        /*$scope.drawCircuitPDU = function()
       {
            //$scope.svg_pduLoad();
            if($rootScope.deviceStatusObj.devicestatus.length === 1){
               $scope.svg_pduLoad(); 
            }
            else if($rootScope.deviceStatusObj.devicestatus.length > 1){
        ...
        $scope.drawCircuit = function(){
            ...
            for(var b=0;b < $rootScope.bcmCount;b++){
                $scope.svg_bcmLoad($rootScope.objBCM.devciestatus,$rootScope.bcmDeviceLabelArr[b]);
            }
            for(var sfStatus = 0;sfStatus <= $rootScope.SFCMSubfeedStatusArr.length - 1;sfStatus++){
                $scope.svg_sfcmLoad($rootScope.SFCMSubfeedStatusArr[sfStatus].status,$rootScope.SFCMSubfeedStatusArr[sfStatus].subFeedId);
            }
        }
        ...
        $scope.invokeDeviceStatusService = function() {
               return $http({
                ...
                url : "http://" + HOSTNAME + ':' + PORT + "/getdata.cgi?devicestatus"
                      ...
                      $rootScope.deviceStatusObj = response.data;
                      console.log(response.data);
                      console.log($rootScope.deviceStatusObj.devicestatus.length);
                      for(var i = $rootScope.deviceStatusObj.devicestatus.length - 1;i >=0; i--){
                            switch($rootScope.deviceStatusObj.devicestatus[i].Type)
                            {
                                case "PDU":
                                $rootScope.objPDU = $rootScope.deviceStatusObj.devicestatus[i];
                                break;
                                case "BCM":
                                $rootScope.objBCM = $rootScope.deviceStatusObj.devicestatus[i];
                                    console.log($rootScope.objBCM);
                                break;
                                case "SFCM":
                                $rootScope.objSFCM = $rootScope.deviceStatusObj.devicestatus[i];
       ...
       //service or function to draw the SVG mimic: END
        ...
        /*$scope.invokeDeviceStatusServicePromise = $interval(function () {
            $scope.invokeDeviceStatusService();
        }, DATADELAY);

        $scope.$on('$destroy', function () {
            $interval.cancel($scope.invokeDeviceStatusServicePromise);
        });*/
        
        /*$timeout(function () {
            $scope.drawCircuitPDU();
        }, 5000);
