...
app.controller('newHomeController', function ($scope, $http, $timeout, $location, $interval, $rootScope) {
    ...
    $scope.identifySFCM = function(sfcmDevice) {
    	...
    	{
            var sfcmPanelRequestURI = 'http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelConfig/submeters/'+sfcmDevice.Id+'/'+parseInt(panelId);
    		$http({
                method: "GET",
                url: sfcmPanelRequestURI
            }).then(function mySuccess(sfcmPanelResponse){
                console.log(sfcmPanelResponse.data);
                var isSFCMPanelOn = $scope.parseSFCMPanelResponseOnOff(sfcmPanelResponse);
                $scope.identifySubFeedOnCTScale(subFeedId, panelId);
            }, function myError(sfcmPanelResponse){
               $scope.SFCMdata = sfcmPanelResponse.data;
            });
    ...
    $scope.parseSFCMPanelResponseOnOff = function(sfcmPanelResponse) {
    	console.log(sfcmPanelResponse.Config["On-Off"]);
        return sfcmPanelResponse.Config["On-Off"] === "On";
    }

    $scope.identifySubFeedOnCTScale = function(subFeedId, sfcmPanelOnId) {
		for (var i = 1; i <= 14; i++)
		{
            var sfcmPanelFactoryConfigURI = 'http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+subFeedId+'/'+sfcmPanelOnId+'/'+parseInt(i);
			$http({
                method: "GET",
                url: sfcmPanelFactoryConfigURI
            }).then(function mySuccess(sfcmPanelFactoryConfigResponse){
                var isCTScaleNonZero = $scope.parseSFCMPanelFactoryConfigResponse(sfcmPanelFactoryConfigResponse);
    				isCTScaleNonZero ? $rootScope.sfcmCount++ : $rootScope.sfcmCount;
                console.log("sfcm count:" +$rootScope.sfcmCount);
            }, function myError(sfcmPanelFactoryConfigResponse){
               $scope.SFCMPanelConfigData = sfcmPanelFactoryConfigResponse.data;
            });
		}
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              $rootScope.bcmCount++;
              console.log("bcm count1:" +$rootScope.bcmCount);
          }
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              $rootScope.bcmCount++;
              console.log("bcm count2:" +$rootScope.bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              $rootScope.bcmCount++;
              console.log("bcm count3:" +$rootScope.bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              $rootScope.bcmCount++;
              console.log("bcm count4:" +$rootScope.bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
    ...
    $scope.invokeDeviceStatusService = function () {
		$http({
            ...
            url : "http://" + HOSTNAME + ':' + PORT + "/getdata.cgi?devicestatus"
              ...
              $scope.identifySFCM($rootScope.objSFCM);
            }, function myError(response) {
   ...
   /*$scope.someFunction = function () {
        .then(function (deviceStatusObj) {
           console.log("length:" +deviceStatusObj.length); 
        });
    }
   $scope.someFunction();*/
 });
