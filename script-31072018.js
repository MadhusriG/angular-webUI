...
//	$rootScope.sfcmCount = 0;
    ...
    {   
        ...
        function identifySubFeedOnCTScale(sfcmPanelOnId, subfeed) {
                ...
                console.log("Subfeed counter:"+subfeed);
                $http({
                        method: "GET",
                        url: 'http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/24/'+parseInt(sfcmPanelOnId)+'/'+parseInt(subfeedNum)
                    }).then(function mySuccess(response){
                        $scope.SFCMPanelSubFeed = response.data;
                        console.log("SF:"+$scope.SFCMPanelSubFeed);
                        //var testCTScale = $scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0;
                        if($scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0){
                           console.log("Condition value is:" +$scope.SFCMPanelSubFeed.SubfeedFactoryConfig.LineCTScale != 0);
                           sfcmCount++; 
                           console.log("sf count:" +sfcmCount);
                        }
                    }, function myError(response){
                       $scope.SFCMPanelSubFeed = response.data;
                    });
...
//        for(var subfeedNum = 1;subfeedNum <= 14; subfeedNum++){
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              bcmCount++;
              console.log("bcm count1:" +bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              bcmCount++;
              console.log("bcm count2:" +bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              bcmCount++;
              console.log("bcm count3:" +bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
...
//          console.log("BCM dta:" +response.data);
          ...
          if($scope.BCM.PanelConfig.OnOff === "On"){
              bcmCount++;
              console.log("bcm count4:" +bcmCount);
          }
        }, function myError(response) {
          $scope.BCM = response.data;
      });
    ...
    $scope.invokeDeviceStatusService = function () {
              ...
              for(var i = $scope.deviceStatusObj.data.devicestatus.length - 1;i >=0; i--){
                    switch($scope.deviceStatusObj.data.devicestatus[i].Type)
                    {
                        ...
                        $rootScope.objBCM = $scope.deviceStatusObj.data.devicestatus[i];
                            console.log("BCM Object:" +$rootScope.objBCM);
                        break;
                        case "SFCM":
                        $rootScope.objSFCM = $scope.deviceStatusObj.data.devicestatus[i];
                            console.log("SFCM Object:" +$rootScope.objSFCM);
                        break;
                    }
                }
        ...
        /* if BCMs and SFCMs are not connected, draw PDU */
//        if(bcmCount === 0 && sfcmCount === 0){
//          $scope.svg_pdu();  
//        }
        /* BCMs and SFCMs are also connected with PDU, draw PDU with connector, BCM, SFCM */
//        else{
            /* create svgs: START */
            svg_pduWithLineLoad();
            for(var b=0;b < 1;b++){
