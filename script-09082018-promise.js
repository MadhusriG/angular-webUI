    ...
    app.controller('newHomeController', function ($scope, $rootScope, $http, $q, $timeout, $location, $interval) {
        ...
        $scope.invokeDeviceStatusService = function() {
               ...
               var deviceStatus = $q.all(promises).then(function(results){
                  $scope.networkFailure = false;
                  $rootScope.deviceStatusObj = results[0].data;
                  ...
                  for(var i = $rootScope.deviceStatusObj.devicestatus.length - 1;i >=0; i--){
                        ...
                        {
                            case "PDU":
                            objPDU = $rootScope.deviceStatusObj.devicestatus[i];
                            break;
                            case "BCM":
                            objBCM = $rootScope.deviceStatusObj.devicestatus[i];
                            break;
                            case "SFCM":
                            objSFCM = $rootScope.deviceStatusObj.devicestatus[i];
                            break;
                        }
                    }
                   return $rootScope.deviceStatusObj;
                   }, function(results) {
                      $scope.networkFailure = true;
                  });
...
//                  identifySFCM($rootScope.objSFCM);
       ...
       //1. function to get the response of the devices connected: END
        ...
        $scope.invokeDeviceStatusService()
            ...
            console.log("second row of circuit");
            $scope.identifySFCM(objSFCM);
            console.log("come on yar!");
            $scope.identifyBCM(objBCM);
            console.log("BCM");
            console.log("SFCM");
            });
