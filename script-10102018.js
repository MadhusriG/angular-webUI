    // Apps
    var app = angular.module('app', ['xeditable','ngRoute', 'ngCookies' , 'angularRangeSlider', 'chart.js']);
    ...
    app.factory('AuthenticationService', function (Base64, $http, $cookieStore, $rootScope, $timeout, $interval) {
            var service = {};

            service.Login = function (username, password, callback) {

                /* Dummy authentication for testing, using $timeout to simulate api call
                 ----------------------------------------------*/
                /*$timeout(function(){
                ...
                $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?Guilogin?user='+username+',pwd='+password)
            ...
            //function to authenticate the user logged in with the auth passkey: START 
            service.authenticateLoginPassKey = function(username){
                $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?Guiloginuser?user='+username+',passcode=dummy@123') 
                        .success(function (response){
                            console.log(response);
                            $rootScope.loginUserResponse = response.Guiloginuser;
                            /*if(username === $rootScope.loginUserResponse.user){
            ...
            //function to authenticate the user logged in with the auth passkey: END
            service.promiseAuthenticateLoginPassKey = function(username) {
                $interval(function(username){										
                    service.authenticateLoginPassKey(username);											
                }, 2000);
            }
    ...
    // Login Controller
    app.controller('LoginController', function($scope, $http, $timeout, $location, $rootScope, $interval, AuthenticationService) {
        ...
        $scope.login = function () {
            ...
            AuthenticationService.Login($scope.inputUsername, $scope.inputPassword, function(response) {
                if(response.success) {
                    AuthenticationService.promiseAuthenticateLoginPassKey($scope.inputUsername);
                    AuthenticationService.SetCredentials($scope.inputUsername, $scope.inputPassword);
                    $location.path('/home');
                } else {
    ...
    // Main Controller
    app.controller('MainController', function($scope, $http, $timeout, $location, $rootScope, $interval, $window) {		
        $rootScope.titleHeader = '';
        $rootScope.Day = '';
        $rootScope.Meridiem = '';
        ...
        $scope.invokeHomeServices = function() {
            ...
            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SysConfig')
                .success(function(response) {
                    ...
                    switch($scope.sysConfigDetails.Audio)
                        {
                            case 'On':
                                $scope.sysConfigAudio = 'UI_Audio_256';
                                break;
                            case 'Off':
                                $scope.sysConfigAudio = 'UI_Audio_Off_256';
                                break;
                        }
                }).catch(function (response){
                    $scope.networkFailure=true;
        ...
        Chart.pluginService.register({
          beforeDraw: function(chart) {
                ...
                if(chart.config.type == "doughnut" && chart.chart.canvas.id == "pduPieChart"){
                    ...
                    ctx.font = fontSize1 + "em HelveticaNeueETPro-Light";
                    ctx.textBaseline = "middle";
                    ctx.fillStyle = '#d2d2d2';

                    var fontSize2 = (height / 57).toFixed(2);
                    ctx.font = fontSize2 + "em HelveticaNeueETPro-Medium";

                    var text1 = $scope.kW.Total,
                            textX = Math.round((width - ctx.measureText(text1).width)/2),
                            textY = height/2*1.5;
                    ctx.fillText(text1, textX, textY);

                    var fontSize3 = (height / 114).toFixed(2);
                    ctx.font = fontSize3 + "em HelveticaNeueETPro-Light";
        ...
        $scope.invokeTimeService = function() {
            ...
            //MS - API call to fetch the time: START
            $http.get('http://' + HOSTNAME2 + ':' + PORT + '/getdata.cgi?meters?datetime')
                 .success(function(response) {
                    var dateAndTime = response.DateTime.Time;
                    var time = dateAndTime.split(":");
                    $rootScope.Time = time[0] + ":" + time[1];
                }).catch(function (response){
                    $scope.networkFailure=true;
                });
            //MS - API call to fetch the time: END
        }
       ...
       // Route to Active Events Page from Dashboard
       $scope.routeActiveEvents = function(){
           $rootScope.activeEventsDisplay = true;
       }

       // Route to Home Page when the back button is clicked: START
       //Route to Meters home page when clicked from pages inside Meters Home: START
       $scope.routeMetersHomePage = function(){
           //$rootScope.routeFlow = {'home': 'home' , 'meters': '', 'settings': ''};
           //$location.url('/metershome');
           $window.history.back();
       }
       //Route to Meters home page when clicked from pages inside Meters Home: END
       ...
       $scope.invokeStatusService = function() {
        ...
        //to hide and show the right side info bar on mobile and tablet devices
//        $( ".cross-icon" ).hide();
//        $( ".mobile-menu" ).hide();
    ...
    app.controller('newHomeController', function ($scope, $rootScope, $http, $q, $timeout, $location, $interval) {
        ...
        var sfcmPromises = [];
        var sfPromises = [];
        var sfStatusPromises = [];
        
        $rootScope.homeMimic = function(){
            console.log("clicked on home");
            $scope.home = !$scope.home; 
        }
        $rootScope.metersHome = function(){
            console.log("clicked on meters");
            $scope.meters = !$scope.meters; 
        }
        $rootScope.settingsHome = function(){
            console.log("clicked on settings");
            $scope.settings = !$scope.settings; 
        }
        $rootScope.aboutHome = function(){
            console.log("clicked on about");
            $scope.about = !$scope.about; 
        }
        $rootScope.helpHome = function(){
            console.log("clicked on help");
            $scope.help = !$scope.help; 
        }
        
        $scope.bcmDeviceLabel = function(bcmDeviceId, bcmPanelOnId)
        {
            //console.log("bcm device label array length:"+$rootScope.bcmDeviceLabelArr.length);
            for(var bcmPanelOn = 0;bcmPanelOn < bcmPanelOnArr.length;bcmPanelOn++){
              ...
              console.log("BCM Label:"+$rootScope.bcmDeviceLabel);
                //$rootScope.bcmDeviceLabelArr.length = 0; 
        ...
        { 
            ...
            bcmPanel.forEach(function(b){
                //console.log(b);
                //console.log("bcm promises array lenght:"+bcmPromises.length);
            ...
            var bcmPanelOnCount = $q.all(bcmPromises).then(function(results){
                ...
                $scope.bcmDeviceLabel(bcmDevice.Id,bcmPanelOnArr);
                //console.log(bcmCount);
                return bcmPanelOnArr;
            },function(results) {
        ...
        {   
            ...
            sfcmPanel.forEach(function(s){
                //console.log(s);
                //console.log("sfcm promises length:"+sfcmPromises.length);
        ...
        function pushsubFeedNumTest(sfcmPanel,subfeedNum){
                for(var i = 0;i <= subfeeds.Subfeed.length-1;i++){
                    if(parseInt(sfcmPanel) === parseInt(subfeeds.Subfeed[i].panelId)){
                      //subfeeds.Subfeed[i].finalSubFeed.length = 0;  
        ...
        $scope.identifySFOnCTScale = function(sfcmDevice,sfcmPanelOnId){
            ...
            for(var sfcmPanelOn = 0;sfcmPanelOn <= subfeeds.Subfeed.length-1;sfcmPanelOn++){ 
                subfeedNum.forEach(function(sf){
                    //console.log(sf);
                    //console.log("sf on CT Scale promises length:"+sfPromises.length);
                    sfPromises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+parseInt(sfcmDevice.Id)+'/'+parseInt(subfeeds.Subfeed[sfcmPanelOn].panelId)+'/'+sf))
                });
            }
            var sfCountOnCTScale = $q.all(sfPromises).then(function(results){
                console.log(results.length);
                for(var i=0;i <= results.length -1;i++){
              ...
              for(var i=0;i <= subfeeds.Subfeed.length-1;i++){
                  for(var j=0;j <= subfeeds.Subfeed[i].finalSubFeed.length-1;j++){ 
                    ...
                    //promises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters'+'/'+parseInt(sfcmDevice.Id)+'/'+parseInt(sfcmPanelOnId[sfcmPanelOn])+'/'+sf))
                      //console.log("sf status promises length:"+sfStatusPromises.length);
                    sfStatusPromises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters'+'/'+parseInt(sfcmDevice.Id)+'/'+parseInt(subfeeds.Subfeed[i].panelId)+'/'+parseInt(subfeeds.Subfeed[i].finalSubFeed[j].subFeedId)));
                  }
                }
            var sfStatus = $q.all(sfStatusPromises).then(function(results){
                console.log(results.length);
                for(var i=0;i <= results.length -1;i++){
        ...
        //function to get the response for the tooltip info on hover over PDU: START
        $scope.invokeTooltipInfoService = function(){
            $http.get("http://" + HOSTNAME + ':' + PORT + "/getdata.cgi?meters?VoltageIN")
            .success(function(response){
               $scope.voltageIN = response.LL;
                console.log($scope.voltageIN);
            })
        ...
        //function to get the response for the tooltip info on hover over PDU: END
       ...
       //1. function to get the response of the devices connected: END
        ...
        $scope.svg_pduLoad = function(deviceStatus){
                //$("#pdu-container").empty();
                ...
                //device status color: START
                switch(deviceStatus){
                    case 'ALARM':
                    svg_pdu.setAttribute("style","stroke:red;stroke-width:0.65;");
                    break;
                    case 'WARNING':
                    svg_pdu.setAttribute("style","stroke:orange;stroke-width:0.65;");
                    break;
                    case 'NORMAL':
                    svg_pdu.setAttribute("style","stroke:green;stroke-width:0.65;");
                    break;
                    case 'UNKNOWN':
                    svg_pdu.setAttribute("style","stroke:none;stroke-width:none;");
                    break;     
                }
                //device status color: END
        ...
        $scope.svg_pduWithLineLoad = function(deviceStatus){
            ...
            /***********pdu top group*****************/
            var pduTopGroup = document.createElementNS("http://www.w3.org/2000/svg", 'g');
            pduTopGroup.setAttribute("id","pduTop");
            svg_pduWithLine.append(pduTopGroup);
            var pduTopLine = document.createElementNS("http://www.w3.org/2000/svg", 'rect');
            pduTopLine.setAttribute("y","23.490372");                   
            pduTopLine.setAttribute("x","103.16511");
            pduTopLine.setAttribute("height","37.759335");
            pduTopLine.setAttribute("width","0.99060559");
            pduTopLine.setAttribute("style","display:inline;fill:#ffffff;");
            pduTopGroup.append(pduTopLine);
            /***********pdu top left curves***********/
            var pduTopLeftCurveGroup = document.createElementNS("http://www.w3.org/2000/svg", 'g'); //Create a defs in SVG's namespace
            ...
            pduTopGroup.append(pduTopLeftCurveGroup);
            var topLeftCurve1 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            topLeftCurve1.setAttribute("d","m 86.303367,108.07398 h 1.591604 a 8.8974716,11.489009 0 0 0 8.274541,9.28542 l 0.450124,0.0149 a 8.8974716,11.489009 0 0 0 8.727094,-9.30021 h 1.58877 a 10.467615,13.495026 0 0 1 -10.315864,11.30576 l -0.529456,-0.0169 a 10.467615,13.495026 0 0 1 -9.786813,-11.28892 z"); //Set path's data
            topLeftCurve1.setAttribute("style","display:inline;opacity:1;fill:#ffffff;fill-opacity:1;"); //Set path's styles
            var topLeftCurve2 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            ...
            pduTopGroup.append(pduTopRightCurveGroup);
            var topRightCurve1 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            topRightCurve1.setAttribute("d","m 86.303367,108.07398 h 1.591604 a 8.8974716,11.489009 0 0 0 8.274541,9.28542 l 0.450124,0.0149 a 8.8974716,11.489009 0 0 0 8.727094,-9.30021 h 1.58877 a 10.467615,13.495026 0 0 1 -10.315864,11.30576 l -0.529456,-0.0169 a 10.467615,13.495026 0 0 1 -9.786813,-11.28892 z"); //Set path's data
            topRightCurve1.setAttribute("style","display:inline;opacity:1;fill:#ffffff;fill-opacity:1;"); //Set path's styles
            var topRightCurve2 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            ...
            /***********pdu bottom part***********/
            var pduBottomGroup = document.createElementNS("http://www.w3.org/2000/svg", 'g');
            pduBottomGroup.setAttribute("id","pduBottom");
            svg_pduWithLine.append(pduBottomGroup);
            //pdu bottom center line
            var pduBottomLine = document.createElementNS("http://www.w3.org/2000/svg", 'rect');
            pduBottomLine.setAttribute("y","86.88958");                   
            pduBottomLine.setAttribute("x","102.76957");
            pduBottomLine.setAttribute("height","37.759335");                    
            pduBottomLine.setAttribute("width","0.99060559");
            pduBottomLine.setAttribute("style","display:inline;fill:#ffffff;");
            pduBottomGroup.append(pduBottomLine);
            /***********pdu bottom left curves***********/
            var pduBottomLeftCurveGroup = document.createElementNS("http://www.w3.org/2000/svg", 'g'); //Create a defs in SVG's namespace
            pduBottomLeftCurveGroup.setAttribute("id","pdu-bottom-left-curve"); //Set g's id
            pduBottomLeftCurveGroup.setAttribute("transform","matrix(-0.49953529,0,0,-0.46557154,127.53089,137.73486)");
            pduBottomGroup.append(pduBottomLeftCurveGroup);
            var bottomLeftCurve1 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            bottomLeftCurve1.setAttribute("d","m 86.303367,108.07398 h 1.591604 a 8.8974716,11.489009 0 0 0 8.274541,9.28542 l 0.450124,0.0149 a 8.8974716,11.489009 0 0 0 8.727094,-9.30021 h 1.58877 a 10.467615,13.495026 0 0 1 -10.315864,11.30576 l -0.529456,-0.0169 a 10.467615,13.495026 0 0 1 -9.786813,-11.28892 z"); //Set path's data
            bottomLeftCurve1.setAttribute("style","display:inline;opacity:1;fill:#ffffff;fill-opacity:1;"); //Set path's styles
            var bottomLeftCurve2 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            ...
            pduBottomGroup.append(pduBottomRightCurveGroup);
            var bottomRightCurve1 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            bottomRightCurve1.setAttribute("d","m 86.303367,108.07398 h 1.591604 a 8.8974716,11.489009 0 0 0 8.274541,9.28542 l 0.450124,0.0149 a 8.8974716,11.489009 0 0 0 8.727094,-9.30021 h 1.58877 a 10.467615,13.495026 0 0 1 -10.315864,11.30576 l -0.529456,-0.0169 a 10.467615,13.495026 0 0 1 -9.786813,-11.28892 z"); //Set path's data
            bottomRightCurve1.setAttribute("style","display:inline;opacity:1;fill:#ffffff;fill-opacity:1;"); //Set path's styles
            var bottomRightCurve2 = document.createElementNS("http://www.w3.org/2000/svg", 'path');
            ...
            pduBottomGroup.append(pduYLetter);
            var pduYLetterText = document.createElementNS("http://www.w3.org/2000/svg",'tspan');
            //pduYLetterText.setAttribute("id",'pduLabelText');
            pduYLetter.append(pduYLetterText);
            ...
            pduConnectorLine.setAttribute("height","0.59938949");                    
            pduConnectorLine.setAttribute("width","500.52371");
            ...
            //to place a tooltip on mouse over on the pdu with line: START
            var htmlMouseTooltip1 = document.getElementById('svgTooltipTop');
            var tooltip = document.getElementsByClassName('tooltip');
            console.log(tooltip);
            $('#pduTop')
                .on("mouseover", function(event){
                tooltip[0].style.opacity = "1";
                tooltip[0].style.color = this.getAttribute("fill");
                var x = (event.pageX);
                var y = (event.pageY);
                htmlMouseTooltip1.style.left = Math.max(0, x - 150) + "px";
                htmlMouseTooltip1.style.top = (y - 150) + "px";
            })
                /*.on("mousemove", function(event){
                //var x = (event.pageX - $('#pdu-container').offset().left) + $(window).scrollLeft();
                var x = (event.pageX);
                //var y = (event.pageY - $('#pdu-container').offset().top) + $(window).scrollTop();
                var y = (event.pageY);
                htmlMouseTooltip1.style.left = Math.max(0, x) + "px";
                htmlMouseTooltip1.style.top = (y) + "px";
            })*/
                .on("mouseout", function(event){
                return tooltip[0].style.opacity = "0";
            });
            var htmlMouseTooltipBtmLeft = document.getElementById('svgTooltipBottomLeft');
            var htmlMouseTooltipBtmRight = document.getElementById('svgTooltipBottomRight');
            $('#pduBottom')
                .on("mouseover", function(event){
                tooltip[1].style.opacity = "1";
                tooltip[2].style.opacity = "1";
                tooltip[1].style.color = this.getAttribute("fill");
                tooltip[2].style.color = this.getAttribute("fill");
                var x = (event.pageX);
                var y = (event.pageY);
                htmlMouseTooltipBtmLeft.style.left = Math.max(0, x - 100) + "px";
                htmlMouseTooltipBtmLeft.style.top = (y - 50) + "px";
                htmlMouseTooltipBtmRight.style.left = Math.max(0, x) + "px";
                htmlMouseTooltipBtmRight.style.top = (y - 50) + "px";
            })
                .on("mouseout", function(event){
                    tooltip[1].style.opacity = "0";
                    tooltip[2].style.opacity = "0";
            });
            /*$('#pdu-bottom-right-curve')
                .on("mouseover", function(event){
                tooltip[2].style.opacity = "1";
                tooltip[2].style.color = this.getAttribute("fill");
                var x = (event.pageX);
                var y = (event.pageY);
                htmlMouseTooltipBtmRight.style.left = Math.max(0, x) + "px";
                htmlMouseTooltipBtmRight.style.top = (y) + "px";
            })
                .on("mouseout", function(event){
                return tooltip[2].style.opacity = "0";
            }); */
            //to place a tooltip on mouse over on the pdu with line: END
        ...
        $scope.drawCircuitBCM = function(){
            for(var b=0;b < bcmPanelOnArr.length;b++){
        ...
        $scope.invokeDeviceStatusService()
            ...
            if(deviceStatusObj.devicestatus.length === 1){
               $scope.svg_pduLoad(objPDU.devciestatus); 
            }
            else if(deviceStatusObj.devicestatus.length > 1){
                $scope.invokeTooltipInfoService();
        ...
        //for polling the response on a time interval of 5seconds
        $scope.newHomePromise = 
            $interval(function(){
                $("#pdu-container").empty();
                $("#bs-container").empty();
                bcmPanelOnArr.length = 0;
                bcmPromises.length = 0;
                $rootScope.bcmDeviceLabelArr.length = 0; 
                subfeeds.Subfeed.length = 0;
                sfcmPromises.length = 0;
                sfPromises.length = 0;
                sfStatusPromises.length = 0;
                $scope.invokeDeviceStatusService()
                    .then(function(data){
                    console.log(objPDU);
                    console.log(objBCM);
        ...
        $scope.$on('$destroy', function () {
            $interval.cancel($scope.newHomePromise);
        });
    ...
    app.controller('metersHomeController', function($scope, $http, $q, $timeout, $location, $interval, $rootScope, $route) {
        $interval.cancel($rootScope.dumpStatusPromise);
        $interval.cancel($rootScope.upgradeStatusPromise);
        ...
        var selectedDeviceInfo = element.currentTarget.firstElementChild.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.nextElementSibling.firstElementChild.firstElementChild.innerText.split('/');
		$rootScope.selectedDeviceId = selectedDeviceInfo[0];
        $rootScope.selectedDevicePanel = selectedDeviceInfo[1];
        console.log(deviceSelected);
        console.log($rootScope.selectedDeviceId);
        console.log($rootScope.selectedDevicePanel);
		switch(deviceSelected)
		{
			case 'PDU':
				$rootScope.routeFlow = {'meters': 'pdumeteringmeters', 'settings': 'pdumeteringsettings'};
				$location.url('/pdumeteringmeters');
				break;
			case 'RPP':
			case 'BCM':
				$rootScope.routeFlow = {'home': 'bcmhome' , 'meters': 'metersBranch', 'settings': 'pdumeteringsettings'};
				$location.url('/bcmhome');
				break;
			case 'SFCM':
				$rootScope.routeFlow = {'home': 'sfcmhome' , 'meters': 'sfcmmeters', 'settings': 'pdumeteringsettings'};
				$location.url('/sfcmmeters');
				break;
            }
        };
          ...
          beforeDraw: function(chart) {
                ...
                if(chart.config.type == "doughnut" && chart.chart.canvas.id == "pduPieChart"){
                    ...
                    var text2 = "kW",
                            textX = Math.round((width - ctx.measureText(text2).width)/2),
                            textY = height/2*2;
                    pieChartUnitLabel.innerText = text2;
        ...
        $scope.refreshLoadCharts = function(){
          $route.reload(); 
        }
        ...
        /*$scope.promiseDeviceMetersService = $interval(function(){										
                                            $scope.invokeDeviceMetersService();											
                                        }, DATADELAY);

        $scope.$on('$destroy', function () {
            $interval.cancel($scope.promiseDeviceMetersService);
        });*/
    ...
    app.controller('SfcmMetersController', function($scope, $http, $timeout, $interval, $rootScope, $location, $rootScope) {
        ...
        $scope.subfeedDetails = function($event, i) {
            ...
            switch($scope.subfeedNumber){
                    case "Subfeed 1":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+1)
                            .success(function(response) {
                    ...
                    break;
                    case "Subfeed 2":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+2)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 3":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+3)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 4":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+4)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 5":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+5)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 6":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+6)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 7":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+7)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 8":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+8)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 9":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+9)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 10":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+10)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 11":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+11)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 12":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+12)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 13":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+13)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
                    ...
                    break;
                    case "Subfeed 14":
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+14)
                            .success(function(response) {	
                                    $scope.subFeedDetails = response;
        ...
        $scope.subfeedDetailsMinView = function($event, i) {
            ...
            switch($scope.subfeedNumber){
                    case "Subfeed 1":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+1)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+1)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails1 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 2":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+2)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+2)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails2 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 3":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+3)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+3)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails3 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 4":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+4)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+4)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails4 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 5":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+5)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+5)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails5 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 6":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+6)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+6)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails6 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 7":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+7)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+7)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails7 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 8":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+8)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+8)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails8 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 9":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+9)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+9)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails9 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 10":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+10)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+10)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails10 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 11":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+11)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+11)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails11 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 12":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+12)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+12)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails12 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 13":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+13)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+13)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails13 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
                    case "Subfeed 14":
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+14)
                            .success(function(response) {	
                                    $scope.subfeedUserConfigDetails = response;
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedFactoryConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+14)
                            .success(function(response) {	
                                    $scope.subfeedFactoryConfigDetails14 = response;							
                            }).catch(function (response){
                                    $scope.networkFailure=true;
                            });
                    break;
        ...
        $scope.saveChanges = function(){
            ...
            switch($scope.subfeedNumber){
                    case "Subfeed 1":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+1, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 2":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+2, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 3":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+3, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 4":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+4, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 5":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+5, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 6":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+6, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 7":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+7, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 8":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+8, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 9":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+9, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 10":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+10, postBody, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 11":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+11, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 12":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+12, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 13":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+13, postBody)
                            .success(function(response) {	
                                    console.log(response);
                    ...
                    break;
                    case "Subfeed 14":
                        $http.post('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedUserConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+14, postBody)
                            .success(function(response) {	
                                    console.log(response);
        ...
        $scope.invokeServices = function(){	
            ...
            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?SubfeedMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedDevicePanel+'/'+1)
                ...
                switch($scope.subFeeds[feed].Others.Status){
                                                                        case "Alarm":
                                                                            $scope.subFeeds[feed].Others.Status = 'AlarmIcon';
                                                                            break;
                                                                        case "Warning":
                                                                            $scope.subFeeds[feed].Others.Status = 'WarningIcon';
                                                                            break;
                                                                        case "Normal":
                                                                            $scope.subFeeds[feed].Others.Status = 'UI_Normal';
                                                                            break;
                                                                    }
                                                    ...
                                                    .success(function(response) {						
                                                        ...
                                                        .success(function(response) {						
                                                            ...
                                                            .success(function(response) {						
                                                                ...
                                                                //to assign the status icon to each subfeed card
                                                                /*for(var i=0;i <= $scope.subFeeds.length-1;i++){
                                                                    switch($scope.subFeeds[i].Others.Status){
                                                                        case "Alarm":
                                                                            $scope.subFeeds[i].Others.Status = 'AlarmIcon';
                                                                            break;
                                                                        case "Warning":
                                                                            $scope.subFeeds[i].Others.Status = 'WarningIcon';
                                                                            break;
                                                                        case "Normal":
                                                                            $scope.subFeeds[i].Others.Status = 'UI_Normal';
                                                                            break;
                                                                    }
                                                                }*/
    ...
    //BCM Controllers
    app.controller('BcmHomeController', function($scope, $http, $timeout, $interval, $rootScope, $location) {
        $interval.cancel($rootScope.dumpStatusPromise);
        $interval.cancel($rootScope.upgradeStatusPromise);
                ...
                $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterLoad/submeters/'+$rootScope.selectedDeviceId+'/1').
                    success(function(response){
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeter/submeters/'+$rootScope.selectedDeviceId+'/1').
                            ...
                            //API call for Other Panel Metering Data like Power kW, Power kVA, Power PF, kWH
                            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterPower/submeters/'+$rootScope.selectedDeviceId+'/1').
                            success(function(response){
                                $scope.panelMeterResponse1 = response;
                                $scope.panelMeterResponseValue1 = JSON.parse(JSON.stringify($scope.panelMeterResponse1));
                            }).catch(function(response){
                                $scope.networkFailure=true;
                            });
                ...
                $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterLoad/submeters/'+$rootScope.selectedDeviceId+'/2').
                    success(function(response) {
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeter/submeters/'+$rootScope.selectedDeviceId+'/2').
                            ...
                            //API call for Other Panel Metering Data like Power kW, Power kVA, Power PF, kWH
                            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterPower/submeters/'+$rootScope.selectedDeviceId+'/2').
                            success(function(response){
                                $scope.panelMeterResponse2 = response;
                                $scope.panelMeterResponseValue2 = JSON.parse(JSON.stringify($scope.panelMeterResponse2));
                            }).catch(function(response){
                                $scope.networkFailure=true;
                            });
                            }).catch(function (response){
                ...
                $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterLoad/submeters/'+$rootScope.selectedDeviceId+'/3').
                    success(function(response) {
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeter/submeters/'+$rootScope.selectedDeviceId+'/3').
                            ...
                            //API call for Other Panel Metering Data like Power kW, Power kVA, Power PF, kWH
                            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterPower/submeters/'+$rootScope.selectedDeviceId+'/3').
                            success(function(response){
                                $scope.panelMeterResponse3 = response;
                                $scope.panelMeterResponseValue3 = JSON.parse(JSON.stringify($scope.panelMeterResponse3));
                            }).catch(function(response){
                                $scope.networkFailure=true;
                            });
                            }).catch(function (response){
                ...
                $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterLoad/submeters/'+$rootScope.selectedDeviceId+'/4').
                    success(function(response) {
                        ...
                        $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeter/submeters/'+$rootScope.selectedDeviceId+'/4').
                            ...
                            //API call for Other Panel Metering Data like Power kW, Power kVA, Power PF, kWH
                            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?PanelMeterPower/submeters/'+$rootScope.selectedDeviceId+'/4').
                            success(function(response){
                                $scope.panelMeterResponse4 = response;
                                $scope.panelMeterResponseValue4 = JSON.parse(JSON.stringify($scope.panelMeterResponse4));
                            }).catch(function(response){
                                $scope.networkFailure=true;
                            });
                            }).catch(function (response){
        ...
        $scope.selectBcmPanelBranch = function($event, i){
            var element = $event;
            var panelSelected = element.currentTarget.firstElementChild.firstElementChild.firstElementChild.firstElementChild.firstElementChild.innerText.split(" ");
            $rootScope.selectedBcmPanel = panelSelected[1];
            console.log(panelSelected);
            console.log($rootScope.selectedBcmPanel);
            $location.url('/metersbranch');
        }
    ...
    app.controller('metersBranchController', function($scope, $http, $q, $timeout, $location, $interval, $rootScope, $compile, $window){
        $rootScope.activeEventsDisplay = false;
        $scope.branchNumbers = [];
        $scope.numOfCols = 3;
        ...
        //to navigate from this view to the bcm home page: START
        $scope.routeBcmHomePage = function(){
            $window.history.back();
        }
        //to navigate from this view to the bcm home page: END
        ...
        $scope.svg_bcmBranchRight = function(number,status){
            ...
            var $element = $('<div class="col-xs-6" id="branchRightWrapper" ng-click="branchInfo($event);"></div>').appendTo('#brRow');
            ...
            //to highlight the branch on selection: START
            if($scope.branchIsClicked){
               branchNumSpan.setAttribute("class","highlightBranch"); 
            }
            //to highlight the branch on selection: END
        }
        ...
        $scope.drawBcmPanelBranch = function(panelBranchNum,panelBranchState,panelBranchStatus){
            ...
            $scope.highlightBranch = function(e){
              console.log(e);
              var branch = e.currentTarget;
              $scope.branchSvg = e.currentTarget.firstElementChild.nextElementSibling;
              $scope.branchSvgHandle = $scope.branchSvg.firstElementChild.nextElementSibling;
              var branchSvgHandle = $scope.branchSvg.firstElementChild.nextElementSibling;
                console.log($scope.branchSvg);
                console.log($scope.branchSvgHandle);
                var branchNumSpan = e.currentTarget.firstChild;
                console.log($scope.clicked);
                if($scope.clicked){
                    branchSvgHandle.setAttribute('style','fill:#0b74da;');
                    branch.setAttribute('style','border:1px solid #222;');
                    branchNumSpan.setAttribute('style','color:#222;font-weight:bold;');
                }
                else if(!$scope.clicked){
                   branchSvgHandle.setAttribute('style','fill:#fff;');
                   branch.setAttribute('style','border:1px solid #ccc;');
                   branchNumSpan.setAttribute('style','color:#ccc;font-weight:normal;'); 
                }
                else{
                    console.log($scope.clicked);
                }
              //$scope.branchSvgHandle.style.fill = '#0b74da';
            ...
            //function to clear the style on a branch before adding to another: START
            $scope.clearBranchStyle = function(e){
                console.log(e);
                $scope.branch = e.currentTarget;
                $scope.branchSvg = e.currentTarget.firstElementChild.nextElementSibling;
                $scope.branchSvgHandle = $scope.branchSvg.firstElementChild.nextElementSibling;
                $scope.branchNumSpan = e.currentTarget.firstChild;
                $scope.branchSvgHandle.style.fill = '#ffffff';
                $scope.branch.style.border = none;
                $scope.branchNumSpan.style.color = '#cccccc';
                $scope.branchNumSpan.style.fontWeight = 'normal';
            }
            //function to clear the style on a branch before adding to another: END
            ...
            $scope.branchInfo = function(ele) {
                ...
                var branch = ele.currentTarget;
                $scope.branchSvg = ele.currentTarget.firstElementChild.nextElementSibling;
                $scope.branchSvgHandle = $scope.branchSvg.firstElementChild.nextElementSibling;
                var branchSvgHandle = $scope.branchSvg.firstElementChild.nextElementSibling;
                var branchNumSpan = ele.currentTarget.firstChild;
                console.log($scope.branchSvg);
                console.log($scope.branchSvgHandle);
                if($scope.branchIsClicked){
                    branchSvgHandle.setAttribute('style','fill:#0b74da;');
                ...
                //console.log($scope.clicked);
                /*if($scope.branchIsClicked === true){
                    branchSvgHandle.setAttribute('style','fill:#0b74da;');
                    branch.setAttribute('style','border:1px solid #222;');
                    branchNumSpan.setAttribute('style','color:#222;font-weight:bold;');
                }
                else if($scope.branchIsClicked === false){
                   branchSvgHandle.setAttribute('style','fill:#222;');
                   branch.setAttribute('style','border:1px solid #ccc;');
                   branchNumSpan.setAttribute('style','color:#ccc;font-weight:normal;'); 
                }
                else{
                    console.log($scope.branchIsClicked);
                }*/
        ...
        $scope.bcmPanelBranch = function(){
            ...
            console.log($scope.branchNumbers);
               var deferred = $q.defer();
               var promises = [];
               promises.push($http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?BranchMeterGrouped/submeters'+'/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedBcmPanel));
    ...
    app.controller('BcmMetersBranchViewController', function($scope, $http, $timeout, $interval, $rootScope) {
        ...
        $scope.invokeServices = function(branch){
            // API call for BCM Meters Details
            //$http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?BranchMeter/submeters/'+$rootScope.selectedDeviceId+'/'+$scope.selectedPanel+'/'+$rootScope.selectedMeterNumberBcmMeters)
            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?BranchMeter/submeters'+'/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedBcmPanel+'/'+$rootScope.selectedBranchNumber)
            .success(function(response) {						
                $scope.branchDetailsCurrent = response.Current;
                $scope.branchDetailsPower = response.Power;
            ...
            // API call for BCM Settings Details
            //$http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?BranchConfig/submeters/'+$rootScope.selectedDeviceId+'/'+$scope.selectedPanel+'/'+$rootScope.selectedMeterNumberBcmMeters)
            $http.get('http://' + HOSTNAME + ':' + PORT + '/getdata.cgi?meters?BranchConfig/submeters'+'/'+$rootScope.selectedDeviceId+'/'+$rootScope.selectedBcmPanel+'/'+$rootScope.selectedBranchNumber)
                .success(function(response) {
                    $scope.branchInfo = response;
                    if($scope.branchInfo.State.OnOff == 'On'){
                        $scope.branchInfo.State.OnOff = true;
    ...
    app.controller('PduMeteringMetersController', function($scope, $http, $timeout, $interval, $rootScope, $location) {
        ...
        $scope.outputView();
        //close button click function, to redirect from this page to Meters Home page
        $scope.backToMetersHome = function(){
            $location.url('/metersHome');
        }
        $scope.promise1 = $interval(function () {
                                    $scope.outputView();
