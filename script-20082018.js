    ...
    app.controller('metersHomeController', function($scope, $http, $q, $timeout, $location, $interval, $rootScope) {
        ...
        $scope.deviceObjArr = {
            ...
            //"devicePanelOnLoad": $scope.devPanelOnLoadData
        }
                ...
                if(deviceObjects[i].devicePanelOn.length != 0){
                  for(var j=0;j< deviceObjects[i].devicePanelOn.length;j++){
                    $scope.deviceObjArr = {
                        ...
                        //"devicePanelOnLoad": deviceObjects[i].devicePanelOn[j].PanelOnLoad
                    }
        ...
        $scope.options = {
						...
						scales: {
                            ...
                            yAxes: [{
                                gridLines: {display: false, /*grid lines of y axis are gone.*/ }
                            }]
                       },
        ...
        //to set colors to the charts 
        /*$scope.colors = [
            {
                backgroundColor: ['#ff0000', '#00ff00', '#0000ff','#ff0000', '#00ff00', '#0000ff','#ff0000', '#00ff00', '#0000ff','#ff0000', '#00ff00', '#0000ff','#ff0000', '#00ff00']
            }
        ];*/
        ...
        $scope.bcmLoadChartData = function(devices){
            $scope.checkPanelNum = function(panelNum,panelData){
            ...
            var bcmPanelOnData = $q.all(promises).then(function(results){
                ...
                for(var i=0;i <= results.length -1;i++){
                    ...
                    $scope.checkPanelNum(panelOnId,$scope.bcmPanelData);
                }
        ...
        $scope.sfcmLoadChartData = function(devices){
                ...
                for(var i=0;i <= devices.length-1;i++){
                    if(devices[i].deviceType === 'SFCM'){
                      for(var j=0;j <= devices[i].devicePanelOn.length-1;j++){
                        console.log("Panel number from the http req:"+panelNum);
                        console.log(devices[i]);
                        console.log(devices[i].devicePanelOn[j].PanelOnId);
                        if(parseInt(panelNum) === parseInt(devices[i].devicePanelOn[j].PanelOnId)){
            ...
            $scope.sfcmPanelDataAvg = function(sfNum,sfcmPanelId,sfcmPanelLoadData){
                ...
                //console.log(loadAvgArr);
                $scope.pushSfcmPanelLoadAvg(sfNum,sfcmPanelId,loadAvg);
            }
            $scope.sfcmPanelSubfeedStatus = function(sfNum,sfcmPanelId,sfStatus){
                for(var i=0;i <= devices.length-1;i++){
                    if(devices[i].deviceType === 'SFCM'){
                      for(var j=0;j <= devices[i].devicePanelOn.length-1;j++){
                        console.log("Panel number from the http req:"+sfcmPanelId);
                        console.log(devices[i]);
                        console.log(devices[i].devicePanelOn[j].PanelOnId);
                        if(parseInt(sfcmPanelId) === parseInt(devices[i].devicePanelOn[j].PanelOnId)){
                          ...
                          for(var c=0;c <= devices[i].devicePanelOn[j].PanelOnStatus.length-1;c++){
                              ...
                              if(parseInt(sfcmPanelId) === parseInt(devices[i].devicePanelOn[j].PanelOnId)){
                                  var num = parseInt(devices[i].devicePanelOn[j].PanelOnId);
                                  console.log(num);
                                  $scope.colors = devices[i].devicePanelOn[j].PanelOnStatus;
                              }
                          }
                      }
                    }   
            ...
            var sfcmPanelOnData = $q.all(promises).then(function(results){
                //generateColors array with a function: START
                ...
                //generateColors array with a function: END
                console.log(results.length);
                for(var i=0;i <= results.length -1;i++){
                    ...
                    console.log(panelOnId);
                    $scope.sfcmPanelDataAvg(subfeedId,panelOnId,sfcmPanelData);
                    $scope.sfcmPanelSubfeedStatus(subfeedId,panelOnId,sfcmSubFeedStatus);
                    //$scope.checkPanelNum(panelOnId,$scope.bcmPanelData);
                }
