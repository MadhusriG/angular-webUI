        ...
        $scope.drawCircuitSFCM = function(){
            /*$scope.editFinalSubfeedArr = function(){
                for(var i=0;i <= subfeeds.Subfeed.length-1;i++){
                    ...
                    newFinalSubFeedArr = subfeeds.Subfeed[i].finalSubFeed.slice(0,8);
                    console.log(newFinalSubFeedArr);
                }
            subfeeds.Subfeed.forEach(function(sf){
                console.log(sf.finalSubFeed.length);
                var newFinalSubFeedArr = [];
                newFinalSubFeedArr = sf.finalSubFeed.slice(0,8);
                console.log(newFinalSubFeedArr);
            })
                //console.log(subfeeds);
        }
            ...
            for(var sfPanel=0;sfPanel < subfeeds.Subfeed.length;sfPanel++){
                for(var sf = 0;sf < 8;sf++){ $scope.svg_sfcmLoad(subfeeds.Subfeed[sfPanel].finalSubFeed[sf].subFeedStatus,subfeeds.Subfeed[sfPanel].finalSubFeed[sf].subFeedId);
                }
            }
        }
    ...
    app.controller('metersHomeController', function($scope, $http, $q, $timeout, $location, $interval, $rootScope) {
        ...
        $scope.invokeDeviceMetersService = function(){
               ...
               var deviceStatus = promise.then(function(results){
                  ...
                  for(var i = deviceStatusObj.devicestatus.length - 1;i >=0; i--){
                        ...
                        deviceObj = {
                                "deviceType": deviceStatusObj.devicestatus[i].Type,
                                "deviceId": deviceStatusObj.devicestatus[i].Id,
                                "devicePanelOn": []
                            }
                        devices.Device.push(deviceObj);
                        console.log(devices);
                    }
                   return deviceStatusObj;
                   }, function(results) {
        ...
        $scope.identifyBCMPanel = function(bcmDevice){
            ...
            function pushBcmDevice(deviceId,devicePanel){
                for(var i=0;i <= devices.Device.length-1;i++){
                    if(parseInt(deviceId) === parseInt(devices.Device[i].deviceId)){
                       devices.Device[i].devicePanelOn.push(devicePanel);  
                    }
            ...
            var bcmPanelOn = $q.all(bcmPromises).then(function(results){
                ...
                for(var i=0;i <= results.length -1;i++){
                    ...
                    if(bcmPanelResponse.PanelConfig.OnOff === "On"){
                       ...
                       pushBcmDevice(bcmDevice.Id,panelOnId);
                    }
                }
                console.log(devices);
                return panelOnId;
            ...
            var sfcmPanelOn = $q.all(sfcmPromises).then(function(results){
                ...
                for(var i=0;i <= results.length -1;i++){
                    ...
                    if(sfcmPanelResponse["Config"]["On-Off"] === "On"){
                       ...
                       pushSfcmDevice(sfcmDevice.Id,panelOnId);
                    }
                }
                console.log(devices);
                return panelOnId;
	    ...
	    $scope.invokeDeviceMetersService().then(function(results){
            $scope.identifyBCMPanel(objBCM).then(function(results){
                $scope.identifySFCMPanel(objSFCM);
            });
        });
    });
